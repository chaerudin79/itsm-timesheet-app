...
export default function TicketTable({ tickets, onUpdateTickets, onApproveTickets }) {
  ...
  return (
    <div className="w-full space-y-4">
      {/* Top action row */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 bg-slate-900/40 rounded-xl border border-slate-800/50">
        ...
        {unapprovedTickets.length > 0 && (
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-3.5 py-1.5 bg-[#06B6D4]/15 hover:bg-[#06B6D4]/30 disabled:opacity-40 text-[#06B6D4] rounded-lg border border-[#06B6D4]/30 text-xs font-semibold flex items-center gap-1.5 transition-all disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              className="px-3.5 py-1.5 bg-[#06B6D4] hover:bg-[#06B6D4]/80 text-black rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all"
            >
      ...
      <div className="mb-3">
        <input
          ...
          className="w-full px-4 py-2 border border-slate-800 rounded-lg bg-slate-900/50 text-slate-100 placeholder-slate-500 focus:outline-none focus:border-[#06B6D4] text-sm"
        />
      </div>

      {tickets.length > 0 && (
        <div className="mb-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-slate-400">
          <span>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-slate-900 border border-slate-800 rounded px-2 py-1 text-slate-100 focus:outline-none focus:border-[#06B6D4]"
            >
              {[25, 50, 100].map(size => (
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-800">
        <table className="tickets-table min-w-full divide-y divide-slate-800">
          <thead>
            <tr>
              <th className="w-8"></th>
              <th className="w-8">
                {unapprovedTickets.length > 0 && (
                  <input
                    ...
                    className="rounded border-slate-700 text-[#06B6D4] focus:ring-0 bg-slate-900"
                  />
                )}
              </th>
              <th>No</th>
          ...
          <tbody className="divide-y divide-slate-800/50">
            {sortedTickets.length === 0 ? (
              <tr>
                <td colSpan="18" className="text-center py-8 text-slate-500">
              ...
              visibleTickets.map((ticket, idx) => {
                ...
                return (
                  <Fragment key={ticket.no || idx}>
                    <tr className={`${rowStyle} ${idx % 2 === 0 ? 'bg-white/[0.02]' : ''} hover:bg-white/[0.05] transition-colors duration-150`}>
