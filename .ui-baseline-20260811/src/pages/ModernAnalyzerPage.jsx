...
export default function ModernAnalyzerPage() {
  ...
  const [groqApiKey, setGroqApiKey] = useState(() => {
    ...
    if (stored && !stored.startsWith('gsk_')) {
  ...
  return (
    <>
      ...
      {/* Main Content */}
      <motion.div
        className="flex-1 flex overflow-hidden bg-[#0A0A0B]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.25 }}
      >
        {/* Sessions Sidebar */}
        <div className="w-64 border-r border-white/[0.06] flex flex-col bg-[#111113]">
          {/* Sessions Header */}
          <div className="p-3 border-b border-white/[0.06]">
            <button
              onClick={createSession}
              className="w-full flex items-center justify-center gap-2 py-2 rounded-md text-sm font-medium bg-[#06B6D4] text-black hover:bg-[#06B6D4]/85 transition-colors"
            >
              <Plus className="w-4 h-4" />
              New Session
            </button>
            ...
            {sessions.map((session) => (
              <div
                ...
                className={`group relative rounded-md p-2.5 cursor-pointer transition-colors duration-150 ${activeSessionId === session.id
                  ? 'bg-[#06B6D4]/[0.12] border-l-2 border-[#06B6D4] pl-[9px]'
                  : 'hover:bg-white/[0.05]'
                  }`}
                onClick={() => setActiveSessionId(session.id)}
              >
                <p className={`text-sm font-medium truncate ${activeSessionId === session.id ? 'text-[#06B6D4]' : 'text-slate-300 group-hover:text-white'
                  }`}>
                  {session.title}
                </p>
